...
//TODO: implement realtime folder update and delete functionality
const useSupabaseRealtime = () => {
  const supabase = createClientComponentClient();
  const { dispatch, state, workspaceId: selectedWorskpace } = useAppState();
  const router = useRouter();
  useEffect(() => {
    //channel listens for all event changes from database
    const channel = supabase
      .channel('db-changes')
      .on(
        'postgres_changes',
        //listens for all changes on table files
